...
if __name__ == "__main__":
    ...
    # tactical_strat.get_perf_stats(plot=True,show=True,strat_name="tactical_strategy",compare=False)
     
    vol_carry_strat = vol_carry_main()
    vol_carry_strat.run_simulation(use_vol_target=False)
    vol_carry_strat.run_hypothesis_tests(num_decision_shuffles=100)
    exit()
    vol_carry_strat.get_perf_stats(plot=True,show=True,strat_name="vol_carry_strategy",compare=False)
    ...
    combined.get_perf_stats(plot=True,show=True,strat_name="combined")