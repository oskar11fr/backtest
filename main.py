...
def tactical_main():
    TICKERS = ["SPY", "QQQ", "TLT", "SHY", "XLU", "XLP", "GLD"]

    db = finance_database(database_name="test_db")
    db.import_to_database(tickers=TICKERS)
    tickers, dfs = db.export_from_database()

    strategy = Tactical(insts=tickers, dfs=dfs, start=datetime(2007,1,1), end=datetime(2024,11,12), benchmark="SPY", max_leverage=1.5, portfolio_vol=0.1)
    return strategy
...
if __name__ == "__main__":
    ...
    tactical_strat.get_perf_stats(plot=True,show=False,strat_name="tactical_strategy")

    vol_carry_strat = vol_carry_main()
    vol_carry_strat.run_simulation(use_vol_target=False)
    vol_carry_strat.get_perf_stats(plot=True,show=False,strat_name="vol_carry_strategy")
    ...
    combined = BuyHold(insts=tickers, dfs=dfs, start=datetime(2007,1,1), end=datetime(2024,11,12), portfolio_vol=0.1, max_leverage = 1.)
    combined.run_simulation(use_vol_target=True)
    combined.get_perf_stats(plot=True,show=False,strat_name="combined_strategy")
